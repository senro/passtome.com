                    ...
                    for(var i= 0,L=allLabels.length;i<L;i++){
                        if(allLabels[i].example){
                            ...
                            if(allLabels[i].attr){
                                ...
                                allLabels[i].css=delCssAttr(allLabels[i].css);
                            }else{
                                //没有设置属性的情况
                                allLabels[i].css=parseCss(exampleData);
                                allLabels[i].css=delCssAttr(allLabels[i].css);
                    ...
                    fs.readFile(inputCodeUrl,'utf8',function(err,senroJs){
                        if(err){
                            ...
                            if (fs.existsSync(htmlObj.dir+'css/')) {
                                fs.writeFile(htmlObj.dir+'css/'+htmlObj.fileName+'-senro.css', allCss, function (err) {
                                    log(htmlObj.dir+'css/'+htmlObj.fileName+'-senro.css'+'创建成功！');
                                });
                            }else{
                            ...
                            if (fs.existsSync(htmlObj.dir+'js/')) {
                                fs.writeFile(htmlObj.dir+'js/'+htmlObj.fileName+'.js', allJs, function (err) {
                                    log(htmlObj.dir+'js/'+htmlObj.fileName+'.js'+'创建成功！');
                                });
                            }else{
...
//                    }
            ...
            function delCssAttr(str){
                var attrReg=new RegExp('/\\*([A-Za-z0-9])*\\*/','g');

                return str.replace(attrReg,'');
            }
            ...
            function addAllHtmlInclude(html){
                html=html.replace(/\<\/head\>/g,'    \<link rel=\"stylesheet\" type=\"text/css\" href=\"css/'+htmlObj.fileName+'-senro.css\"\>\n\r\<\/head\>');
                html=html.replace(/\<\/body\>/g,'\<script type=\"text/javascript\" src=\"js/Senro.'+exportList.version+'.js\"\>\</script\>\n\r\<script type=\"text/javascript\" src=\"js/'+htmlObj.fileName+'.js\"\>\</script\>\n\r\<\/body\>');
                return html;
            }
