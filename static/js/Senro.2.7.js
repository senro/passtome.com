...
Senro.prototype={
    ...
    checkRequire:function(type,fun_action){
         ...
         * @返回：  无；
         * @实例：  /test-html/2.7/test-checkRequire.html；
         * @需要：  无；
         * @备注：  暂无；
         */
        var state=false;
    ...
    console:function(str){
         ...
         * @返回：  无；
         * @实例：  /test-html/2.7/test-console.html；
         * @需要：  无；
         * @备注：  暂无；
         */
        (function() {
    ...
    len:function (s) {
         ...
         * @返回：  长度（int）；
         * @实例：  /test-html/2.7/test-len.html；
         * @需要：  无；
         * @备注：  暂无；
         */
        var s=s||'';
    ...
    random:function (min,max){
         ...
         * @返回：  随机数（int）；
         * @实例：  /test-html/2.7/test-random.html；
         * @需要：  无；
         * @备注：  暂无；
         */
    ...
    isIE6:function(){
         ...
         * @实例：  /test-html/2.7/test-isIE6.html；
         * @需要：  无；
         * @备注：  暂无；
         */
    ...
    addScript:function(cus_attrs,$domPos){
         ...
         * @返回：  无；
         * @实例：  /test-html/2.7/test-addScript.html；
         * @需要：  无；
         * @备注：  暂无 todo；
         */
    ...
    sliderTab:function($_btns,$_conts,str_btn_hover,str_trigerType,str_transType,int_currIndex){
         ...
         * @返回：  无；
         * @实例：  /test-html/2.7/test-sliderTab.html；
         * @需要：  无；
         * @备注：  暂无；
         */
        var $btns=$_btns,
    ...
    bigEye:function(str_id_container,str_id_itemsWrap,str_id_dotsWrap,str_dotAction){
        /*
         * @位置： 常用组件；
         * @名字：  bigEye；
         * @翻译：  大眼睛轮播图片（ 容器id，图片包裹层id，导航dot包裹层的id，触发dot的类型 ）；
         * @参数：  bigEye(containerId,itemsWrapId,dotsWrapId,dotAction)
         *         containerId（字符串）：【必填】容器id
         *         itemsWrapId（字符串）：【必填】图片包裹层id
         *         dotsWrapId（字符串）：【必填】导航dot包裹层的id
         *         dotAction（字符串）：【可选】触发dot的类型，‘mouseover’，‘click’，默认click；
         * @功能：  初始化大眼睛轮播图片组件；
         * @返回：  无；
         * @实例：  /test-html/2.7/test-bigEye.html；
         * @需要：  checkRequire；
         * @备注：  暂无 todo 待重写；
         */
        senro.checkRequire('MSClass',function(){
            var $container=$('#'+str_id_container),
                eyeWidth=$container.width(),
                eyeHeight=$container.height();
            var bigEye=new Marquee([str_id_container,str_id_itemsWrap,str_id_dotsWrap,str_dotAction||'click'],2,0.5,eyeWidth,eyeHeight,20,5000,5000,0,2);
            document.getElementById(str_id_container).style.cssText = "";
            document.getElementById(str_id_container).style.cssText = "";
        });
    },//bigEye end
    marquee:function(str_containerId,str_direction,$_preBtn,$_nextBtn,num_speed){
         ...
         * @参数：  marquee( containerId, direction, $preBtn, $nextBtn, speed )
         *         containerId（字符串）：【必填】容器id
         *         direction（字符串）：【必填】滚动方向，‘left’，‘right’，‘top’，‘bottom’
         *         $preBtn（字符串）：【可选】jquery选中的上一个按钮
         *         $nextBtn（字符串）：【可选】jquery选中的下一个按钮
         *         speed（数字）：【可选】滚动速度，默认1；
         * @功能：  箭头控制滚动方向、加速及鼠标拖动实例；
         * @返回：  无；
         * @实例：  /test-html/2.7/test-marquee.html；
         * @需要：  checkRequire；
    ...
    simpleSliderLR:function($_btn_l,$_btn_r,$_list,int_showNum,str_loop,int_speed){
        /*
         * @位置： 常用组件；
         * @名字：  simpleSliderLR；
         * @翻译：  简单的带箭头图片滚动（ 左按钮，右按钮，图片包裹层，展示图片数量，循环，速度 ）；
         * @参数：  simpleSliderLR($btn_l,$btn_r,$list,showNum,loop,speed)
         ...
         * @功能：  初始化带左右箭头的轮播图片组件；
         * @返回：  无；
         * @实例：  /test-html/2.7/test-simpleSliderLR.html；
         * @需要：  无；
         * @备注：  注意在初始化时，该$list里的元素不能被隐藏，否则offset会为0；
         */
    ...
    sliderLR:function($list,$btn_l,$btn_r,$thumbList,$thumbBtn_l,$thumbBtn_r,$sliderCube){
        /*
         * @位置： 常用组件；
         * @名字：  sliderLR；
         * @翻译：  带缩略图，滚动条的图片滚动（ 大图片包裹层，大图左按钮，大图右按钮，小图包裹层，小图左按钮，小图右按钮，滚动条滑块 ）；
         * @参数：  sliderLR( $list, $btn_l, $btn_r, $thumbList, $thumbBtn_l, $thumbBtn_r,  $sliderCube )
         *         $list（$）：【必填】jquery选中的图片包裹层
         ...
         * @返回：  无；
         * @实例：  /test-html/2.7/test-sliderLR.html；
         * @需要：  checkRequire；
         * @备注：  todo 该方法还不完善，2.7的基本没有大问题，某些特殊情况可能会出现的点bug但不影响使用
         *         如果初始状态是隐藏的，建议用visible：hidden隐藏，不然可能里面的position（）获取不到值，导致初始化有错误；
         */
        ...
        var $big_listItem=getItems($big_list),
            maxIndex = $big_listItem.length-1,
            currIndex=0;
        ...
        if($small_list){
            ...
            var $current=$('<div class="thumb_current"></div>');
    ...
    carouselSlider:function($items,$btnLeft,$btnRight){
        /*
         * @位置： 常用组件；
         * @名字：  carouselSlider；
         * @翻译：  环形带箭头图片滚动（ 图片包裹层，左按钮，右按钮 ）；
         * @参数：  carouselSlider( $items, $btnLeft, $btnRight )
         *         $items（$）：【必填】jquery选中的图片包裹层
         *         $btnLeft（$）：【必填】jquery选中的左按钮
         *         $btnRight（$）：【必填】jquery选中的右按钮；
         * @功能：  初始化带缩略图和滚动条的轮播图片组件；
         * @返回：  无；
         * @实例：  /test-html/2.7/test-carouselSlider.html；
         * @需要：  无；
         * @备注：  todo carouselSlider实现的不太好，需完善；
         */
        var $itemsAtt=[],
            count= 0,
            times=[
        ...
        $items.click(function(){
            ...
            if($this.hasClass('current')){
                window.location.href=$this.find('.itemLink').attr('href');
            }else{
                ...
                if(placeIndex==0||placeIndex==1){
                    ...
                    count=nextCount;
                }else if(placeIndex==3||placeIndex==4){
                    var nextCount=count-1;
                    if(nextCount<0){
                        nextCount=4;
                    }
        ...
        function action(direction,count,nextCount){
            for(var j=0;j<=4;j++){
                ...
                }else if(times[count][j]==1||times[count][j]==3){
                    mask=0.55;
                    zIndex=101;
                }else{
                    mask=0;
            ...
            for(var i=0;i<=4;i++){
                ...
                $items.eq(times[count][i]).find('.itemMask').animate({
    ...
    scrollBar:function($scrollCube,$container,str_direction,speed){
         ...
         * @参数：  scrollBar( $scrollCube, $container, direction, speed )
         ...
         * @功能：  初始化带缩略图和滚动条的轮播图片组件；
         * @返回：  无；
         * @实例：  /test-html/2.7/test-scrollBar.html；
         * @需要：  checkRequire；
    ...
    selectWidget:function(str_id,str_iconClass,str_optionSelectedClass,int_maxNum){
         ...
         * @返回：  无；
         * @实例：  /test-html/2.7/test-selectWidget.html；
         * @需要：  checkRequire、scrollBar；
         * @备注：  selectWidget组件需要引入senro.css
        ...
        $select.after(
            '<div id="'+str_id+'" class="senroSelect '+selectClass+'" data-name="'+selectName+'" data-value="'+$selectedOption.val()+'">'+
                ...
                '<div class="senroOptions"><div class="senroSelectScrollLine '+cusScrollLine+'"><div class="senroSelectScrollCube '+cusScrollCube+'"></div></div><div class="senroOptionsWrap"></div></div>'+
                '</div><input class="senroSelectHiddenInput" type="hidden" name="'+selectName+'" value="'+$selectedOption.val()+'"/>'+
                '</div>'
        );
        $select.remove();
        //insert html
        var $senroSelectCont=$('#'+str_id+' .senroSelect-cont'),
            $senroOptionsContainer=$('#'+str_id+' .senroOptions'),
        ...
        //reset
        $senroSelect.add($senroSelectCont).css('width',selectWidth+'px');
    ...
    cssTriangle:function($dom,str_direction,str_width,str_color){
         ...
         * @返回：  无；
         * @实例：  /test-html/2.7/test-cssTriangle.html；
         * @需要：  isIE6；
         * @备注：  无；
         */
    ...
    //常用组件结束
    //常用界面交互方法
    controlPop:function(str_type,$_pop,$_close,$_mask){
        /*
         * @位置： 常用界面交互方法；
         * @名字：  controlPop；
         * @翻译：  控制弹窗（ 类型，弹框主体，关闭按钮，遮罩层 ）；
         * @参数：  controlPop(type,$pop,$close,$mask)
         ...
         * @返回：  无；
         * @实例：  /test-html/2.7/test-controlPop.html；
         * @需要：  无；
         * @备注：  如果弹框主体里有表单重置表单；
         */
        var $mainPop = $_pop,
            ...
            $close=$_close||'',
            $body=$('body'),
            hasMask=arguments[3] ? true : false;
    ...
    addVideo:function(str_id,str_file,str_image,num_width,num_height,callback){
         ...
         * @位置： 常用界面交互方法；
         * @名字：  addVideo；
         * @翻译：  添加视频（ id，视频路径，截图路径，宽度，高度，回调 ）；
         * @参数：  addVideo(id,file,image,width,height,callback)
         *         id（字符串）：【必填】视频dom容器id
         ...
         * @返回：  无；
         * @实例：  /test-html/2.7/test-addxxx.html；
         * @需要：  checkRequire；
    ...
    addSwf:function(str_id,str_file,num_width,num_height,callback){
         ...
         * @位置： 常用界面交互方法；
         * @名字：  addSwf；
         * @翻译：  加载flash（ id，swf文件路径，宽度，高度，回调 ）；
         * @参数：  addSwf(id,file,width,height,callback)
...
//        };
    ...
    addShare:function($dom,str_mainJsId){
         ...
         * @功能：  快速加常见分享；
         * @返回：  无；
         * @实例：  /test-html/2.7/test-addShare.html；
         * @需要：  addScript；
    ...
    backTop:function($btn){
         ...
         * @功能：  快速初始化返回顶部按钮；
         * @返回：  无；
         * @实例：  /test-html/2.7/test-backTop.html；
         * @需要：  isIE6；
         * @备注：  无；
         */
    ...
    addCurrent:function($btns,str_currName){
         ...
         * @返回：  无；
         * @实例：  /test-html/2.7/test-addCurrent1.html；
         * @需要：  无；
    ...
    checkLogin:function(fun_logined,fun_notLogin){
         ...
         * @功能：  检测登录状态；
         * @返回：  无；
         * @实例：  /test-html/2.7/test-checkLogin.html；
         * @需要：  无；
    ...
    loginOut:function($_a){
         ...
         * @功能：  给注销按钮添加注销链接；
         * @返回：  无；
         * @实例：  /test-html/2.7/test-loginOut.html；
         * @需要：  无；
         * @备注：  按钮必须为a标签；
         */
    ...
    getJson:function (url,obj_argus,callback){
         ...
         * @功能：  给注销按钮添加注销链接；
         * @返回：  无；
         * @实例：  /test-html/2.7/test-getJson.html；
         * @需要：  无；
         * @备注：  argus 参数示例
         *         var argus={
         *              argu1: argu1Value,
...
//        }
    ...
    initLogin:function(formId,scope,successCallback){
         ...
         * @功能：  给注销按钮添加注销链接；
         * @返回：  无；
         * @实例：  /test-html/2.7/test-initLogin.html；
         * @需要：  checkRequire；
         * @备注：  todo，尽量减少传参，让方法通过formid自己去加后面的参数，要不然使用起来太复杂,待测试
         *         参数
         *         username 	    必选 	用户名
         *         password 	    必选 	密码
         *         jumpurl 	        可选 	跳转路径
         ...
         */
        //senro.console('正常输入的类名：inputDefault，提示框的类名：msgContainer，用户名参数：username，密码：password。');
        senro.checkRequire('login',function(){

            var input_node='.inputDefault',
                input_write='.inputWrite',
                input_done='.inputComplete',
                msg_node='.msgContainer',
                msg_hover='.msgHover',
                msg_error='.msgError';
            //add default input class
            var $inputs=$('#'+formId).find('input').filter(':text');
            $inputs.eq(0).addClass('inputDefault').attr('name','username');
            $inputs.eq(1).addClass('msgContainer');
            $inputs.eq(2).addClass('inputDefault').attr('name','password');
            $inputs.eq(3).addClass('msgContainer');
            var login = new Login({
                formId:formId,
                elements:{
                    scope:scope,
    ...
    placeHolderInput:function($inputs,$submit){
         ...
         * @返回：  无；
         * @实例：  /test-html/2.7/test-placeHolderInput.html；
         * @需要：  无；
         * @备注：  无；
         */
        var values=[];
    ...
    toggleInput:function($normalInput,$msgInput){
         ...
         * @功能：  快速初始化带提示的输入框；
         * @返回：  无；
         * @实例：  /test-html/2.7/test-toggleInput.html；
         * @需要：  无；
         * @备注：  暂无；
         */
    ...
    checkInput:function($form,fun_callback,str_msgsClassName){
         ...
         * @参数：  checkInput( $form, callback, msgsClassName )
         *         $form（$）：【必填】jquery选中的需要验证的表单
         *         callback（函数）：【可选】检测全部通过后的回调函数
         *         msgsClassName（字符串）：【可选】提示框的类名，默认：msg;
         * @功能：  表单验证，可以检测的类型：require、email、cellPhone、chinaID、password，只需在输入框的类上加上相应的类型即可；
         * @返回：  无；
         * @实例：  /test-html/2.7/test-checkInput.html；
         * @需要：  无；
    ...
    checkHtml5:function(type,fun_support_callback,fun_notSupport_callback){
         ...
         * @返回：  无；
         * @实例：  /test-html/2.7/test-checkHtml5.html；
         * @需要：  checkRequire；
         * @备注：  todo 多类型同时检测；
         */
    ...
    fixedDomPos:function($dom){
         ...
         * @返回：  无；
         * @实例：  /test-html/2.7/test-fixedDomPos.html；
         * @需要：  isIE6；
    ...
    fixIE6Png:function(str_className){
         ...
         *         className（字符串）：【必填】需要修复的dom类名;
         * @功能：  修复ie6的png问题；
         * @返回：  无；
         * @实例：  /test-html/2.7/test-fixIE6Png.html；
         * @需要：  checkRequire；
         * @备注：  该修复可能导致的问题：
    ...
    load:function(arr_paths,fun_fileLoad,fun_Progress,fun_onComplete){
         ...
         * @返回：  无；
         * @实例：  /test-html/2.7/test-load.html；
...
//            }
    ...
    tweenColor:function($dom,obj_attr,num_time){
         ...
         * @功能：  缓动颜色；
         * @返回：  无；
         * @实例：  /test-html/2.7/test-tweenColor.html；
         * @需要：  checkRequire；
         * @备注：  无；
         */
    ...
    addGradient:function($dom,str_config){
         ...
         * @功能：  给dom添加渐变；
         * @返回：  无；
         * @实例：  /test-html/2.7/test-addGradient.html；
         * @需要：  checkRequire；
    ...
    css3Rotate:function($dom,num_deg){
         ...
         * @功能：  js旋转dom，兼容ie6；
         * @返回：  无；
         * @实例：  /test-html/2.7/test-css3Rotate.html；
         * @需要：  checkRequire；
    ...
    css3Translate:function($dom,num_x,num_y){
         ...
         * @返回：  无；
         * @实例：  /test-html/2.7/test-css3Translate.html；
         * @需要：  checkRequire；
         * @备注：  todo 多属性同时调用的覆盖问题；
         */
        var objDom=$dom[0],
    ...
    css3Scale:function($dom,num_x,num_y){
         ...
         * @返回：  无；
         * @实例：  /test-html/2.7/test-css3Scale.html；
         * @需要：  checkRequire；
         * @备注：  todo 多属性同时调用的覆盖问题；
         */
        var objDom=$dom[0],
    ...
    css3Skew:function($dom,num_x,num_y){
         ...
         * @返回：  无；
         * @实例：  /test-html/2.7/test-css3Skew.html；
         * @需要：  checkRequire；
         * @备注：  todo 多属性同时调用的覆盖问题；
         */
        var objDom=$dom[0],
    ...
    css3SetOpacity:function($dom,num_n){
         ...
         * @返回：  无；
         * @实例：  /test-html/2.7/test-css3SetOpacity.html；
         * @需要：  checkRequire；
         * @备注：  todo 多属性同时调用的覆盖问题；
         */
        var objDom=$dom[0],
    ...
    css3SetTransform:function($dom,str_config){
         ...
         * @返回：  无；
         * @实例：  /test-html/2.7/test-css3SetTransform.html；
         * @需要：  checkRequire；
    ...
    directionAnimate:function($container,$mask){
         ...
         * @返回：  无；
         * @实例：  /test-html/2.7/test-directionAnimate.html；
         * @需要：  无；
         * @备注：  无；
         */
    ...
    tween:function(obj,num_time,tweenObj,num_delay,str_easeType,fun_onUpdate,arr_onUpdateParams,fun_onComplete,arr_onCompleteParams,fun_callback){
         ...
         * @功能：  缓动对象属性；
         * @返回：  无；
         * @实例：  /test-html/2.7/test-tween.html；
         * @需要：  checkRequire；
         * @备注：  todo 待优化；
         */
