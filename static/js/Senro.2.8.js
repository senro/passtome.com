...
Senro.prototype={
    ...
    sliderTab:function($_btns,$_conts,str_btn_hover,str_trigerType,str_transType,int_currIndex){
         ...
         * @返回：  无；
         * @实例：  /test-html/2.8/widget/sliderTab.html；
         * @需要：  无；
         * @备注：  暂无；
         */
        var $btns=$_btns,
    ...
    bigEye:function($itemWrap,$dotsWrap,str_btn_hover,str_trigerType,str_transType,str_autoPlay,int_time){
         ...
         * @参数：  bigEye($itemWrap,$dotsWrap,btn_hover,trigerType,transType,autoPlay,time)
         *         $itemWrap（字符串）：【必填】jquery选中的图片包裹层
         *         $dotsWrap（字符串）：【必填】jquery选中的bullet包裹层
         *         btn_hover（字符串）：【必填】current样式名
         ...
         * @功能：  初始化大眼睛轮播图片组件；
         * @返回：  无；
         * @实例：  /test-html/2.8/widget/bigEye.html；
         * @需要：  getItems；
         * @备注：  暂无 todo 待重写；
         */
    ...
    marquee:function(str_containerId,str_direction,$_preBtn,$_nextBtn,num_speed){
         ...
         * @返回：  无；
         * @实例：  /test-html/2.8/widget/marquee.html；
         * @需要：  checkRequire；
    ...
    simpleSliderLR:function($_btn_l,$_btn_r,$_list,int_showNum,str_loop,int_speed,str_autoPlay,int_autoPlayTime){
        /*
         * @位置： 常用组件；
         * @名字：  simpleSliderLR；
         ...
         * @功能：  初始化带左右箭头的轮播图片组件；
         * @返回：  无；
         * @实例：  /test-html/2.8/widget/simpleSliderLR.html；
         * @需要：  getItems；
    ...
    sliderLR:function($list,$btn_l,$btn_r,$thumbList,$thumbBtn_l,$thumbBtn_r,$sliderCube,str_current){
         ...
         * @参数：  sliderLR( $list, $btn_l, $btn_r, $thumbList, $thumbBtn_l, $thumbBtn_r,  $sliderCube,  current )
         *         $list（$）：【必填】jquery选中的图片包裹层
         ...
         * @返回：  无；
         * @实例：  /test-html/2.8/widget/sliderLR.html；
         * @需要：  checkRequire；
    ...
    carouselSlider:function($items,$btnLeft,$btnRight,str_maskName){
        /*
         * @位置： 常用组件；
         * @名字：  carouselSlider；
         * @翻译：  环形带箭头图片滚动（ 图片包裹层，左按钮，右按钮，遮罩类名 ）；
         * @参数：  carouselSlider( $items, $btnLeft, $btnRight，maskName )
         *         $items（$）：【必填】jquery选中的图片包裹层
         *         $btnLeft（$）：【必填】jquery选中的左按钮
         *         $btnRight（$）：【必填】jquery选中的右按钮
         *         maskName（字符串）：【选填】默认为‘itemMask’；
         * @功能：  初始化带缩略图和滚动条的轮播图片组件；
         * @返回：  无；
         * @实例：  /test-html/2.8/widget/carouselSlider.html；
         * @需要：  无；
...
//                [4,0,1,2,3]
        ...
        $items.click(function(){
            ...
            if($this.hasClass('current')){
                ...
                if(placeIndex==0||placeIndex==1){
                    ...
                    count=nextCount;
                }else if(placeIndex==3||placeIndex==itemsLength){
                    var nextCount=count-1;
                    if(nextCount<0){
        ...
        function action(direction,count,nextCount){
            ...
            for(var j=0;j<=itemsLength;j++){
                var mask=$items.eq(times[count][j]).data('mask'),//考虑如何去掉这步从css获取
                    zIndex=parseInt($items.eq(times[count][j]).data('zindex'));
                $itemsAtt[j]=
                {
                    mask: mask,
            ...
            for(var i=0;i<=itemsLength;i++){
                if(times[nextCount][i]==Math.floor((0+itemsLength)/2)){
    ...
    scrollBar:function($scrollCube,$container,str_direction,speed){
         ...
         * @功能：  初始化带缩略图和滚动条的轮播图片组件；
         * @返回：  无；
         * @实例：  /test-html/2.8/widget/scrollBar.html；
         * @需要：  checkRequire；
    ...
    selectWidget:function(str_id,str_iconClass,str_optionSelectedClass,int_maxNum){
         ...
         * @返回：  无；
         * @实例：  /test-html/2.8/widget/selectWidget.html；
...
//                '</div>' +
    ...
    cssTriangle:function($dom,str_direction,str_width,str_color){
         ...
         * @返回：  无；
         * @实例：  /test-html/2.8/widget/cssTriangle.html；
         * @需要：  isIE6；
         * @备注：  无；
         */
    ...
    controlPop:function(str_type,$_pop,$_close,$_mask){
         ...
         * @返回：  无；
         * @实例：  /test-html/2.8/UIInteractive/controlPop.html；
         * @需要：  无；
         * @备注：  如果弹框主体里有表单重置表单；
         */
        var $mainPop = $_pop,
...
//        }
    ...
    initLogin:function(formId,scope,successCallback){
         ...
         * @功能：  给注销按钮添加注销链接；
         * @返回：  无；
         * @实例：  /test-html/2.8/dataInteractive/initLogin.html；
         * @需要：  checkRequire；
    ...
    checkInput:function($form,fun_callback,str_msgsClassName){
         ...
         * @返回：  无；
         * @实例：  /test-html/2.8/check/checkInput.html；
         * @需要：  无；
    ...
    fixedDomPos:function($dom){
         ...
         * @返回：  无；
         * @实例：  /test-html/2.8/fix/fixedDomPos.html；
         * @需要：  isIE6；
