...
Senro.prototype={
    ...
    bigEye:function($itemWrap,$dotsWrap,str_btn_hover,str_trigerType,str_transType,str_autoPlay,int_time){
         ...
         * @功能：  初始化大眼睛轮播图片组件；
         * @返回：  无；
         * @实例：  /test-html/2.9/widget/bigEye.html；
         * @需要：  getItems；
         * @备注：  可以使用< !--senroLabel.sliderTab[属性名：属性值][...]-- >这样的标签格式调用，目前支持属性：
    ...
    marquee:function(str_containerId,str_direction,$_preBtn,$_nextBtn,num_speed){
         ...
         * @返回：  无；
         * @实例：  /test-html/2.9/widget/marquee.html；
         * @需要：  checkRequire；
    ...
    simpleSliderLR:function($_btn_l,$_btn_r,$_list,int_showNum,str_loop,int_speed,str_autoPlay,int_autoPlayTime,str_touchType){
         ...
         * @位置： 常用组件；
         * @名字：  simpleSliderLR；
         * @翻译：  简单的带箭头图片滚动（ 左按钮，右按钮，图片包裹层，展示图片数量，循环，速度,自动播放,自动播放时间 ，切换类型）；
         ...
         *         touchType(字符)：【可选】触摸类型，默认‘normal’，默认按个数切换，设置成其他任何字符串则是按拖动距离切换；
         * @功能：  初始化带左右箭头的轮播图片组件；
         * @返回：  无；
    ...
    sliderLR:function($list,$btn_l,$btn_r,$thumbList,$thumbBtn_l,$thumbBtn_r,$sliderCube,str_current){
         ...
         * @返回：  无；
         * @实例：  /test-html/2.9/widget/sliderLR.html；
         * @需要：  checkRequire；
    ...
    ,
    carouselSlider:function($items,$btnLeft,$btnRight,str_maskName,currIndex){
        /*
         * @位置： 常用组件；
         * @名字：  carouselSlider；
         ...
         * @功能：  初始化带缩略图和滚动条的轮播图片组件；
         * @返回：  无；
         * @实例：  /test-html/2.9/widget/carouselSlider.html；
         * @需要：  无；
...
//        });
    ...
    scrollBar:function($scrollCube,$container,str_direction,speed){
         ...
         * @功能：  初始化带缩略图和滚动条的轮播图片组件；
         * @返回：  无；
         * @实例：  /test-html/2.9/widget/scrollBar.html；
         * @需要：  checkRequire；
...
//                '</div>' +
    ...
    //常用组件结束
    //常用界面交互方法
    controlPop:function(str_type,$_pop,$_close,$_mask,num_speed){
        /*
         * @位置： 常用界面交互方法；
         * @名字：  controlPop；
         ...
         * @返回：  无；
         * @实例：  /test-html/2.9/UIInteractive/controlPop.html；
         * @需要：  无；
         * @备注：  如果弹框主体里有表单重置表单；
         */
        var $mainPop = $_pop,
    ...
    ,
    addVideo:function(str_id,str_file,str_image,callback){
        /*
         * @位置： 常用界面交互方法；
         * @名字：  addVideo；
         * @翻译：  添加视频（ id，视频路径，截图路径，回调 ）；
         * @参数：  addVideo(id,file,image,callback)
         *         id（字符串）：【必填】视频dom容器id
    ...
    ,
    addSwf:function(str_id,str_file,callback){
        /*
         * @位置： 常用界面交互方法；
         * @名字：  addSwf；
         * @翻译：  加载flash（ id，swf文件路径，回调 ）；
         * @参数：  addSwf(id,file,callback)
