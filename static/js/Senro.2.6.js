...
var Senro=function(requireJs){
    ...
    senro.testHtml5={ state: false, objects: [], action: $.Callbacks(), callbacks: [] };
    senro.preload={ state: false, objects: [], action: $.Callbacks(), callbacks: [] };
        ...
        for(var i=0;i<arr_requireJs.length;i++){
            switch (arr_requireJs[i]){
                ...
                case 'iePng':
                    ...
                    break;
                case 'drag':
                    $.getScript('http://static.woniu.com/script/plugin/jquery-ui-drag.min.js',function(){
                        typeAction('drag');
                    });
                    break;
                case 'mouseWheel':
                    $.getScript('http://static.woniu.com/script/plugin/jquery.mousewheel.js',function(){
                ...
                case 'MSClass':
                    ...
                    break;
                case 'animateColors':
                    $.getScript('http://static.woniu.com/script/plugin/jquery.animate-colors-min.js',function(){
                ...
                case 'tweenLite':
                    ...
                    break;
                case 'testHtml5':
                    $.getScript('http://static.woniu.com/script/plugin/modernizr.custom.testHtml5.js',function(){
                        typeAction('testHtml5');
                    });
                    break;
                case 'preload':
                    $.getScript('http://static.woniu.com/script/plugin/preloadjs-0.3.1.min.js',function(){
                        typeAction('preload');
                    });
                    break;
            }
    ...
    senro.console(
        'Senro.2.6（依赖jquery 1.9.1）'+'\n'+
        '包含方法：'+'\n'+
        'controlPop、getJson、initLogin、checkLogin、loginOut、marquee、simpleSliderLR、sliderLR、' +
        'scrollBar、sliderTab、addNavFooter、addShare、changeFont、addScript、addVideo、addSwf、' +
        'tweenColor、addGradient、css3Rotate、css3Translate、css3Scale、css3Skew、css3SetOpacity、' +
        'css3SetTransform、checkRequire、checkInput、directionAnimate、checkHtml5、load、console、' +
        'selectWidget、cssTriangle、fixedDomPos、backTop、fixIE6Png、len、isIE6'
    );
};
Senro.prototype={
    ...
    initLogin:function(formId,scope,successCallback,input_node,input_write,input_done,msg_node,msg_hover,msg_error){
        /*
        依赖checkRequire方法，todo，尽量减少传参，让方法通过formid自己去加后面的参数，要不然使用起来太复杂
        参数
        * username 	    必选 	用户名
          password 	    必选 	密码
          jumpurl 	    可选 	跳转路径
    ...
    addShare:function($dom){
        ...
        $dom.html(shareHtml);
        senro.addScript(jsAttr);
    },
    changeFont:function($big,$normal,$small,$chgCont){
        //大
        $big.click(function(){
    ...
    addVideo:function(str_id,str_file,str_image,num_width,num_height,callback){
        /*
        * 依赖checkRequire、console方法
        * */
        senro.console('senro.addVideo: 参数str_Id 不需要#');
    ...
    addSwf:function(str_id,str_file,num_width,num_height,callback){
        ...
        */
        senro.console('senro.addSwf:str_Id 不需要#');
    ...
    checkInput:function($form,fun_callback,str_msgsClassName){
        ...
        var consoleStr=
            '可以检测的类型：require、email、cellPhone、chinaID、password'+'\n'+
    ...
    checkHtml5:function(type,fun_support_callback,fun_notSupport_callback){
        /*依赖checkRequire、console方法*/
        senro.console('checkHtml5 type: webgl, canvas, audio, video, csstransforms3d');
    ...
    load:function(arr_paths,fun_fileLoad,fun_Progress,fun_onComplete){
        /*
        *依赖checkRequire、console、addScript方法
        * ie6 只支持图片加载
        * */
        senro.console(
            'Senro.load: 需要服务测试，不然会报错！现代浏览器支持加载很多格式：图片，音频,json,xml。ie只支持图片和js加载\n' +
...
//            }
    ...
    selectWidget:function(str_id,str_iconClass,str_optionSelectedClass){
        /*依赖console、addScript*/
        senro.console('selectWidget组件需要引入senro.css');
        //read infos
        var $select=$('#'+str_id),
            $selectContainer=$select.parent(),
            selectName=$select.attr('name'),
            selectClass=$select.attr('class'),
            selectIconClass=str_iconClass||selectClass+'-icon',
            $options=$select.find('option'),
            optionClass=$options.eq(0).attr('class'),
            optionSelectedClass=str_optionSelectedClass||'senroOption_hover',
            $selectedOption=$options.filter('[selected=selected]'),
            selectedOptionIndex=$selectedOption.index(),
            $head=$('head');
        //add senro.css
//        if(str_senroCssPath){
//            $head.prepend('<link type="text/css" rel="stylesheet" href="'+str_senroCssPath+'" />');
//        }else{
//            $head.prepend('<link type="text/css" rel="stylesheet" href="../css/senro.css" />');
//        }
        $selectContainer.prepend(
            '<div id="'+str_id+'" class="senroSelect '+selectClass+'">'+
        ...
        for(var i=0;i<$options.length;i++){
            if(i==selectedOptionIndex){
                $senroSelectCont.append('<a class="senroOption '+$options.eq(i).attr('class')+'" href="javascript:;" data-select="selected" data-value="'+$options.eq(i).val()+'">'+$options.eq(i).html()+'</a>');
            }else{
                $senroSelectCont.append('<a class="senroOption '+$options.eq(i).attr('class')+'" href="javascript:;" data-value="'+$options.eq(i).val()+'">'+$options.eq(i).html()+'</a>');
            }
        }
        $select.hide();
        ...
        var $senroSelectIcon=$('#'+str_id+' .senroSelect-icon'),
            ...
            $senroOptions=$senroSelectCont.find('.senroOption');
        //reset
        $senroSelect.add($senroSelectCont).css('width',$select.width()+'px');
